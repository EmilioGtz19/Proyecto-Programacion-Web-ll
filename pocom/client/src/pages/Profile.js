...
const Profile = (props) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (firstName && fatherLastName && motherLastName) {
            ...
            if (responseUpdate.ok) {
                swalAlert("Usuario actualizado", "El usuario se ha actualizado satisfactoriamente.", "success")
            } else {
                swalAlert("Error", "Ha ocurrido un error al actualizar el usuario", "error")
            }
    ...
    async function updateUser() {
        try {
            const response = await fetch(`${apiUrl}/api/user/update/${props.user.id}`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
            ...
            <div className="divProfile">
                ...
                <div style={{ width: 550 }} className="App bg-white p-5 rounded-5 text-success shadow-lg">
                    ...
                    <div className=" d-flex justify-content-center gap-5">
                        ...
                        <Form.Group className="mb-3 mt-3">
                            <Form.Label className="fw-semibold">Nombre</Form.Label>
                            <Form.Control className="shadow-sm border border-success border-2" type="text" onChange={e => setFirstName(e.target.value)} value={firstName} />
                            {error && firstName.length <= 0 ?
                                <Form.Label className="col-6 text-danger">El nombre no puede estar vacío</Form.Label> : ""}
                        </Form.Group>
                        ...
                        <Form.Group className="mb-3">
                            <Form.Label className="fw-semibold">Apellido Paterno</Form.Label>
                            <Form.Control className="shadow-sm border border-success border-2" type="text" onChange={e => setFatherLastName(e.target.value)} value={fatherLastName} />
                            {error && fatherLastName.length <= 0 ?
                                <Form.Label className="col-6 text-danger">El nombre no puede estar vacío</Form.Label> : ""}
                        </Form.Group>
                        ...
                        <Form.Group className="mb-3">
                            <Form.Label className="fw-semibold">Apellido Materno</Form.Label>
                            <Form.Control className="shadow-sm border border-success border-2" type="text" onChange={e => setMotherLastName(e.target.value)} value={motherLastName} />
                            {error && motherLastName.length <= 0 ?
                                <Form.Label className="col-6 text-danger">El nombre no puede estar vacío</Form.Label> : ""}
                        </Form.Group>
